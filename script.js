...
function renderServices(services) {
    ...
    grid.innerHTML = services.map(s => `
        <div class="service-card" data-category="${s.category}" data-price="${extractPrice(s.price)}" data-service="${s.title}">
            <div class="service-card-header">
                <div class="service-icon">${s.icon}</div>
                <div class="service-actions">
                    <button class="service-favorite-btn" title="Favorilere Ekle" onclick="toggleFavorite(this, '${s.title}')">❤️</button>
                    <button class="service-cart-btn" title="Sepete Ekle" onclick="addToCart('${s.title}', '${s.price}')">🛒</button>
                </div>
            </div>
            <h3>${s.title}</h3>
            <p>${s.description}</p>
...
// ── 10. FAVORİ VE SEPET SİSTEMİ ─────────────────────────────────────────────
function toggleFavorite(btn, serviceName) {
    let favorites = JSON.parse(localStorage.getItem('favorites')) || [];
    
    if (favorites.includes(serviceName)) {
        favorites = favorites.filter(f => f !== serviceName);
        btn.classList.remove('active');
        btn.style.opacity = '0.6';
        showNotification(`"${serviceName}" favorilerden kaldırıldı.`, 'info');
    } else {
        favorites.push(serviceName);
        btn.classList.add('active');
        btn.style.opacity = '1';
        showNotification(`"${serviceName}" favorilere eklendi! ❤️`, 'success');
    }
...
function addToCart(serviceName, servicePrice) {
    ...
    showNotification(`"${serviceName}" sepete eklendi! 🛒\nSepet: ${cart.length} ürün`, 'success');
}
...
function initWeatherWidget() {
    ...
    if (!weatherBtn || !weatherCity) return;
    
    // Varsayılan şehir (Ankara)
    fetchWeather('Ankara');
...
function fetchWeather(city) {
    const apiKey = '7d0fb5c93d4af21c66a2948710284366'; // OpenWeatherMap API Key
    ...
    fetch(url)
        ...
        .then(data => {
            if (data.cod === 200) {
                ...
                showWeatherError('Şehir bulunamadı!');
            }
        })
        .catch(() => {
            showWeatherError('Hava durumu bilgisi alınamadı!');
        });
}
...
function showCart() {
    ...
    cartHTML += `\nTOPLAM: ${total.toFixed(2)} TL\n\nSepeti temizlemek için "Temizle" butonuna tıklayın.`;
    
    const result = confirm(cartHTML + '\n\nSepeti temizlemek ister misiniz?');
    if (result) {
...
// Sayfa yüklendiğinde hava durumu widget'ını başlat
document.addEventListener('DOMContentLoaded', function () {
